...
import { ArrowLeft, ShieldCheck, AlertTriangle, ChevronRight } from 'lucide-react';
      ...
      <div className="space-y-5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 text-left space-y-3">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${(selectedSub.usagePercentage || 0) > 50 ? 'bg-emerald-500/10 text-emerald-500' : (selectedSub.usagePercentage || 0) > 20 ? 'bg-amber-500/10 text-amber-500' : 'bg-red-500/10 text-red-500'}`}>
        ...
        {selectedSub.status !== 'Cancelled' ? (
          <>
            <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl flex space-x-2.5 items-start text-left mb-2">
              ...
              <p className="text-[11px] text-slate-400">You&apos;ll be charged ₹{selectedSub.cost || 0} {selectedSub.renewDate || 'soon'}. Cancel now via our on-device assistant to avoid it.</p>
            </div>
