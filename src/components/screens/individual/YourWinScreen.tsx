  ...
  return (
    <div className="flex flex-col min-h-full bg-slate-950 text-white p-5 justify-between">
      <div className="space-y-5">
        ...
        <motion.div initial={{ scale: 0.9, y: 20, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <motion.h2 initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 400, damping: 10, delay: 0.3 }}
            className="text-5xl font-black text-emerald-400 tracking-tight font-mono">
            {isAnonymized ? '₹XX,XXX' : `₹${totalSavedYearly.toLocaleString('en-IN')}`}
          </motion.h2>
          <p className="text-xs text-slate-500 font-semibold">this year</p>

          <div className="grid grid-cols-3 gap-2 text-center pt-2">
            <div><p className="text-lg font-black text-white font-mono">{subsCancelled}</p><p className="text-xs text-slate-500 leading-snug">Subscriptions cancelled</p></div>
            <div><p className="text-lg font-black text-white font-mono">{totalScamsBlocked}</p><p className="text-xs text-slate-500 leading-snug">Scams blocked</p></div>
            <div><p className="text-lg font-black text-white font-mono">{Math.max(subsCancelled, totalScamsBlocked, 8)}</p><p className="text-xs text-slate-500 leading-snug">Months protected</p></div>
          </div>

          <div className="pt-2">
            <span className="inline-flex items-center space-x-1 px-3 py-1 bg-slate-950 border border-slate-800 rounded-full text-xs text-slate-500 font-bold uppercase tracking-wider font-mono">
        ...
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl flex justify-between items-center">
          <div className="text-left">
            ...
            <p className="text-xs text-slate-500 mt-0.5">Hide the exact amount in the card</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2.5 text-center">
          <button onClick={() => handleShare('whatsapp')} className="p-3 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition flex flex-col items-center space-y-1.5 focus-visible:ring-2 focus-visible:ring-sky-500" aria-label="Share on WhatsApp">
          ...
          <button onClick={() => handleShare('instagram')} className="p-3 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition flex flex-col items-center space-y-1.5 focus-visible:ring-2 focus-visible:ring-sky-500" aria-label="Share on Instagram">
          ...
          <button onClick={() => handleShare('copy')} className="p-3 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition flex flex-col items-center space-y-1.5 focus-visible:ring-2 focus-visible:ring-sky-500" aria-label="Copy share link">
      ...
      <div className="pb-4">
        <button onClick={() => showToast('success', 'Saving card shared! Keeping your savings streak.')} className="w-full py-4 bg-slate-900 hover:bg-slate-850 text-sky-400 border border-sky-500/20 font-bold rounded-2xl flex items-center justify-center space-x-2 transition active:scale-95 focus-visible:ring-2 focus-visible:ring-sky-500" aria-label="Download as Image">
