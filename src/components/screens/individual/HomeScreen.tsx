...
const textMutedDark = 'text-slate-400';
    ...
    <div className="flex flex-col min-h-full pb-24 p-4 space-y-4 relative bg-slate-950 text-white">
      ...
      {isRefreshing && (
        <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 flex items-center space-x-2 bg-slate-900/90 border border-slate-800 rounded-full px-4 py-2 shadow-lg">
          ...
          <span className="text-[10px] font-bold text-sky-400">Refreshing...</span>
        </div>
      )}
      {isOffline && (
        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[10px] font-bold p-3 rounded-xl flex items-center justify-between shadow-sm" role="alert">
          <div className="flex items-center space-x-2">
            <WifiOff className="w-4 h-4" aria-hidden="true" />
            <span>You're offline. Live scanning paused.</span>
          </div>
          <button onClick={() => navigate('/vault')} className="underline focus-visible:ring-2 focus-visible:ring-amber-500 rounded">Vault Access</button>
        </div>
      )}
      ...
      <div onClick={() => setIsShieldActive(!isShieldActive)} className={`p-5 rounded-[24px] border transition-all duration-300 flex items-center space-x-5 text-left cursor-pointer group select-none ${isShieldActive ? 'bg-[#1a2325] border-emerald-900/40 shadow-sm' : 'bg-slate-900/60 border-slate-800'}`} role="switch" aria-checked={isShieldActive} tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setIsShieldActive(!isShieldActive); }}>
        <div className="relative shrink-0 flex items-center justify-center w-16 h-16">
          {isShieldActive ? (
            <div className="absolute inset-0 rounded-full bg-[#0a1516] flex items-center justify-center shadow-[0_0_15px_rgba(45,212,191,0.2)]" aria-hidden="true">
              <div className="w-[52px] h-[52px] rounded-full border-[1.5px] border-[#153229] flex items-center justify-center">
                <div className="w-[38px] h-[38px] rounded-full border-[1.5px] border-[#1b4334] flex items-center justify-center bg-[#0d1e18] relative">
        ...
        <div className="flex-1 space-y-2 z-10 min-w-0">
          <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full border border-emerald-900/50 bg-[#163a2e]/60">
            ...
            <span className="text-[10px] font-semibold text-[#2dd4bf] tracking-wide">{isShieldActive ? 'Protected' : 'Paused'}</span>
          </div>
          ...
          <p className="text-[11px] leading-tight text-slate-400 font-medium">{isShieldActive ? `${blockedCount} scam${blockedCount !== 1 ? 's' : ''} blocked • 0 threats today` : 'Tap to resume active defense.'}</p>
        </div>
      </div>
      ...
      <GlassCard className="p-4 text-left">
        ...
        <div className="flex justify-between items-end mt-1.5 mb-1">
          <div>
            <AnimatedNumber value={wastedAmount} prefix="₹" className={`text-3xl font-extrabold ${wastedAmount > 0 ? 'text-red-500' : 'text-emerald-500'}`} format />
          ...
          {wastedAmount > 0 ? (
            ...
            <span className="text-xs text-emerald-500 font-bold">All Good!</span>
          )}
        </div>
        ...
        <div className="w-full bg-slate-800/40 rounded-full h-2 mt-3 overflow-hidden">
      ...
      <div className="text-left">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-[10px] bg-sky-500/10 text-sky-400 font-semibold px-2 py-0.5 rounded-full">{unreadCount} new</span>
        </div>
        <motion.div className="flex space-x-3 overflow-x-auto pb-1 scrollbar-none" variants={containerVariants} initial="hidden" animate="show">
          ...
          <motion.div variants={itemVariants} as="div">
            <GlassCard hover onClick={() => { setSelectedSub(subscriptions[0]); navigate('/sub-detail'); }} className="p-4 border-l-4 border-l-red-500 shrink-0 w-44 text-left">
              <AppIcons name="movie" size="sm" />
          ...
          <motion.div variants={itemVariants} as="div">
            <GlassCard hover onClick={() => navigate('/scan-qr')} className="p-4 border-l-4 border-l-amber-500 shrink-0 w-44 text-left">
                ...
                <GlassCard hover onClick={() => navigate('/receipt-dark')} className="p-3 flex items-center justify-between">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    <div className="text-left flex-1 min-w-0">
                      ...
                      <p className={`text-[10px] ${textMuted} truncate`}>{act.description}</p>
                    </div>
                  </div>
                  <span className={`text-[9px] px-2 py-0.5 rounded-full font-semibold ${act.status === 'Blocked' ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}>{act.status}</span>
                </GlassCard>
              </motion.div>
            ))
          )}
