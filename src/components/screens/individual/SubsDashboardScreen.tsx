...
function DonutChart({ subscriptions, total, formatTotal }: { subscriptions: Subscription[]; total: number; formatTotal: (n: number) => string }) {
  ...
  return (
    <div className="flex items-center space-x-6 mt-3 relative z-10">
      ...
      <div className="flex-1 space-y-1.5 text-[10px] font-bold" role="list" aria-label="Category breakdown">
        {segments.map((seg) => (
...
export default function SubsDashboardScreen() {
  ...
  const currentDisplayedSubs = subscriptions;
  ...
  return (
    <div className="flex flex-col min-h-full pb-24 bg-slate-950 text-white">
      ...
      <div className="p-4 rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/60 to-slate-950/80 text-left relative overflow-hidden shadow-xl" role="tabpanel" id="subscription-panel">
        ...
        <DonutChart subscriptions={currentDisplayedSubs} total={monthlyBurn} formatTotal={formatTotal} />
      </div>
      </div>

      <div className="text-left flex-1 flex flex-col min-h-[220px] px-4 pb-4 pt-4">
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3 text-left">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        {isCritical && <span className="bg-red-500/10 text-red-400 text-[8px] font-black px-1.5 py-0.5 rounded border border-red-500/10 uppercase">Critical Waste</span>}
                ...
                <div className="mt-3 flex items-center space-x-3 text-[10px]">
                  ...
                  <span className={`${textColor} shrink-0 font-bold text-[9px] uppercase font-mono`}>{statusLabel} ({sub.usagePercentage}%)</span>
                </div>
              </motion.div>
            );
          })}
