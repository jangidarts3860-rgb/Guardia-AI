...
export default function LoginScreen() {
  ...
  return (
    <div className="flex flex-col justify-between min-h-full bg-transparent text-white p-6 relative overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          {isPanicMode ? (
            <div className="space-y-3 pt-2">
              ...
              <span className="block text-center text-[10px] text-slate-400 font-mono pt-1">Locked for your security</span>
            </div>
          ) : (
            <>
              <div className="flex justify-center space-x-6 py-4" role="status" aria-live="polite" aria-label={`PIN code: ${pinVal.length} digits entered`}>
                {[...Array(4)].map((_, idx) => {
                  ...
                  return (
                    <div key={idx} className={`w-4 h-4 rounded-full transition-all duration-150 ${filled ? 'bg-sky-400 border border-sky-400' : 'border-2 border-slate-800 bg-transparent'}`} />
                  );
                })}
              </div>
              ...
              <div className="w-full max-w-xs px-2 mx-auto pb-4">
                <div className="grid grid-cols-3 gap-y-4 gap-x-5 text-center">
                  {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                    <button key={num} onClick={() => handleNumPress(num.toString())} className="w-14 h-14 rounded-full card-surface text-lg font-black transition-all flex items-center justify-center active:bg-slate-700 active:scale-90 focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:outline-none" aria-label={`Press ${num}`}>
                      {num}
                    </button>
                  ))}
                  <button onClick={triggerBiometric} className="w-14 h-14 rounded-full bg-transparent text-emerald-400 transition-all flex items-center justify-center active:scale-90 active:bg-emerald-500/10 focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none" aria-label="Login with Fingerprint">
                    <Fingerprint className="w-8 h-8" />
                  </button>
                  <button onClick={() => handleNumPress('0')} className="w-14 h-14 rounded-full card-surface text-lg font-black transition-all flex items-center justify-center active:bg-slate-700 active:scale-90 focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:outline-none" aria-label="Press 0">
                    0
                  </button>
                  <button onClick={handleBackspace} className="w-14 h-14 rounded-full bg-transparent text-slate-400 text-xl font-bold transition-all flex items-center justify-center active:bg-slate-800/50 active:scale-90 focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:outline-none" aria-label="Delete last digit">
                    ⌫
                  </button>
                </div>
              </div>
