All Brands Master — Design System Reference
> 59 brand design systems merged into one file.
> Source: awesome-design-md skill library.
> Usage: Paste into VS Code / any AI as context. Reference only — not design law.
> Authority: UX laws from ux-thinking override any brand pattern here.
---
...
Semantic theme tokens: `--theme\_\*` CSS variable naming
2. Color Palette & Roles
Primary
Deep Navy (`#181d26`): Primary text
Airtable Blue (`#1b61c9`): CTA buttons, links
White (`#ffffff`): Primary surface
Spotlight (`rgba(249,252,255,0.97)`): `--theme\_button-text-spotlight`
Semantic
Success Green (`#006400`): `--theme\_success-text`
Weak Text (`rgba(4,14,32,0.69)`): `--theme\_text-weak`
Secondary Active (`rgba(7,12,20,0.82)`): `--theme\_button-text-secondary-active`
Neutral
...
BMW Blue for interactive only — never as background or decoration
Photography carries emotion — the UI is pure precision
Tight line-heights everywhere — 1.15 to 1.30 is the range
...
3. Typography Rules
Font Family
Primary: `Inter` (Next.js optimized variant `\_\_Inter\_d1b8ee`)
Secondary Display: `Basier` (`\_\_basier\_a58b65`), with fallbacks: `Arial, Helvetica`
Code: `Inconsolata` (`\_\_Inconsolata\_a25f62`)
Hierarchy
Role	Font	Size	Weight	Line Height	Letter Spacing	Notes
...
Neon Volt (#faff69) is for accents and CTAs only — never large backgrounds
Weight 900 for hero, 700 for headings, 600 for labels, 400-500 for body
Active states use Pale Yellow (#f4f692) — not just opacity changes
All links hover to Neon Volt — consistent interactive feedback
Charcoal borders (rgba(65,65,65,0.8)) are the primary depth mechanism
...
Iteration Guide
Focus on ONE component at a time
Always use 22px radius for primary cards — "the Cohere card roundness"
Specify the typeface — CohereText for headlines, Unica77 for body, CohereMono for labels
Interactive elements use Interaction Blue (#1863dc) on hover only
Keep surfaces white with cool gray borders — no warm tones
Purple is for full-width sections, never card backgrounds
...
When refining existing screens generated with this design system:
Focus on ONE component at a time
Reference specific color names and hex codes from this document — "use Ghost White (rgba(255,255,255,0.6))" not "make it lighter"
Use natural language descriptions — "make the border barely visible" = Border Mist 04-06
Describe the desired "feel" alongside specific measurements — "compressed and authoritative heading at 48px with line-height 1.0"
For glow effects, specify "Electric Cyan at 12% opacity as a radial gradient behind the element"
Always specify which font — abcDiatype for marketing, JetBrains Mono for technical/code content
...
Waldenburg 300 for headings — never bold, the lightness is the identity
Multi-layer shadows: always include inset + outline + elevation at sub-0.1 opacity
Positive letter-spacing on Inter body (+0.14px to +0.18px) — the airy reading quality
Warm stone CTA is the signature — `rgba(245,242,239,0.8)` with `rgba(78,50,23,0.04)` shadow
Pill (9999px) for buttons, generous radius (16px–24px) for cards
...
Iteration Guide
Focus on ONE component at a time
Reference specific color names and hex codes — "use Slate Gray (#60646c)" not "make it gray"
Use radius values deliberately — 6px for buttons, 8px for cards, 24px for images, 9999px for pills
Describe the "feel" alongside measurements — "enormous breathing room with 96px section spacing"
Always specify Inter and the exact weight — weight contrast IS the hierarchy
For shadows, specify "whisper shadow" or "standard elevation" from the elevation table
Keep the interface monochrome — let product content be the color
...
Overlay transparency: The `--f-color-overlay-darker` at 80% opacity creates depth without shadow
Photographic depth: Studio-lit car imagery with reflections, ground shadows, and atmospheric haze provides all the visual dimensionality
Decorative Depth
...
Reference specific color names and hex codes from this document — the palette is small but each color has a precise role
Use natural language descriptions, not CSS values — "razor-sharp 2px corners" conveys intent better than "border-radius: 2px"
...
Always maintain the chiaroscuro contrast — if a section feels flat, check whether it needs to be on black or white to maintain the alternating rhythm
Reserve Ferrari Red for ONE element per screen — if red appears in more than one place, it loses its authority
...
Iteration Guide
Use variable font weight stops precisely: 320, 330, 340, 450, 480, 540, 700
Interface is always black + white — never add colors to chrome
Dashed focus outlines, not solid
Letter-spacing is always negative on body, always positive on mono labels
Pill (50px) for buttons/tabs, circle (50%) for icon buttons
...
Always verify letter-spacing on GT Walsheim headings — the extreme negative tracking is non-negotiable
Check that Framer Blue appears ONLY on interactive elements — never as decorative background or text color for non-links
Ensure all buttons are pill-shaped — any squared corner immediately breaks the Framer aesthetic
Test frosted glass surfaces by checking they have exactly `rgba(255, 255, 255, 0.1)` — too opaque looks like a bug, too transparent disappears
...
The typography is anchored by a custom brand font (HashiCorp Sans, loaded as `\_\_hashicorpSans\_96f0ca`) that carries substantial weight — literally. Headings use 600–700 weights with tight line-heights (1.17–1.19), creating dense, authoritative text blocks that communicate enterprise confidence. The hero headline at 82px weight 600 with OpenType `"kern"` enabled is not decorative — it's infrastructure-grade typography.
...
3. Typography Rules
Font Families
Primary Brand: `\_\_hashicorpSans\_96f0ca` (HashiCorp Sans), with fallback: `\_\_hashicorpSans\_Fallback\_96f0ca`
...
Icon Font: `ibm\_icons` — proprietary icon glyphs at 20px
Hierarchy
Role	Font	Size	Weight	Line Height	Letter Spacing	Notes
...
Letter-spacing only at small sizes: 0.16px at 14px, 0.32px at 12px — never on display text
Three weights: 300 (display), 400 (body), 600 (emphasis) — no bold
Blue 60 is the only accent color — do not introduce secondary accent hues
Depth comes from background-color layering (white → #f4f4f4 → #e0e0e0), not shadows
Inputs have bottom-border only, never fully boxed
Use `--cds-` prefix for token naming to stay Carbon-compatible
48px is the universal interactive element height
...
Letter-spacing scales with font size: -1.584px at 72px, -1.056px at 48px, -0.704px at 32px, normal below 16px
Three weights: 400 (read), 510 (emphasize/navigate), 590 (announce)
Surface elevation via background opacity: `rgba(255,255,255, 0.02 → 0.04 → 0.05)` — never solid backgrounds on dark
Brand indigo (`#5e6ad2` / `#7170ff`) is the only chromatic color — everything else is grayscale
Borders are always semi-transparent white, never solid dark colors on dark backgrounds
Berkeley Mono for any code or technical content, Inter Variable for everything else
...
Brand green (`#18E299`) is used sparingly — CTAs and hover states only, never for decorative fills
Geist Mono uppercase for technical labels, Inter for everything else
Section padding is generous: 64px–96px on desktop, 48px on mobile
No gray background sections — white throughout, separation through borders and whitespace
...
Iteration Guide
Start with the mode decision: dark (#001e2b) for hero/features, white for content
MongoDB Green (#00ed64) is electric — use once per section for maximum impact
Serif headlines (MongoDB Value Serif) create the editorial authority — never use for body
Weight 300 body text creates the airy reading experience — don't default to 400
Source Code Pro uppercase with wide tracking for technical labels — the database voice
Teal-tinted shadows keep everything in the MongoDB color world
...
Weight 700 is the dominant voice for all interactive and heading elements; 400 is only for body paragraphs
...
Link hover is always `#3860be` (blue) regardless of the link's default color
Line-height 1.25 for headings, 1.50-1.67 for body text -- maintain this contrast for visual hierarchy
Navigation uses uppercase 14px bold -- this hardware-label typography is part of the brand voice
...
Keep surfaces flat: no shadows, no gradients, no blur effects. Use borders and background shifts only.
The warm undertone matters: use `#201d1d` not `#000000`, use `#fdfcfc` not `#ffffff`. The reddish warmth is subtle but essential.
Border radius is 4px everywhere except inputs (6px). Never use rounded pills or large radii.
Semantic colors follow Apple HIG: `#007aff` blue, `#ff3b30` red, `#30d158` green, `#ff9f0a` orange. Each has hover and active darkened variants.
Three-stage interaction: default → hover (darkened) → active (deeply darkened) for all semantic colors.
Borders use `rgba(15, 0, 0, 0.12)` -- a warm transparent dark, not neutral gray. This ties borders to the warm palette.
Spacing follows an 8px grid: 8, 16, 24, 32, 40, 48, 64, 80, 96px. Use 4px for fine adjustments only.
...
Check that all text uses the olive family (#4d4f46, #23251d) not pure black or neutral gray
Ensure hover states flash PostHog Orange (#F54E00) — if hovering feels bland, you're missing this
Confirm borders use sage-tinted gray (#bfc1b7) not neutral gray — warmth runs through every element
The overall tone should feel like a fun, scrappy startup wiki — never corporate-polished or sterile
...
Ensure shadows have both outer and inset layers — single-layer shadows look flat and wrong
Confirm Inter has OpenType features `calt`, `kern`, `liga`, `ss03` enabled
Test that hover states use opacity transitions (0.6) not color swaps — this is a core interaction pattern
...
Reference specific color names and hex codes — the palette is small but each color has a precise function
Use natural language descriptions, not CSS values — "sharp zero-radius rectangle" conveys intent better than "border-radius: 0"
...
Always check whether a section should be light or dark — the chessboard alternation is a core pattern
Reserve Renault Yellow for ONE button per screen — if yellow appears in more than one CTA, the hierarchy collapses
...
Display text is HEAVY — weight 700, sizes 48px+
Links use dotted underline (#bbbbbb) — never solid
The gradient hero is the visual anchor — make it bold
Use basier-square for body, rb-freigeist-neue for display, JetBrains Mono for code
...
Frost borders (`rgba(214, 235, 253, 0.19)`) are the universal structural element — not gray, not neutral
...
Multi-color accents at low opacity (12–42%) for backgrounds, full opacity for text
Pill shape (9999px) for CTAs and badges, standard radius (4px–16px) for containers
No shadows — use frost borders for depth against the void
...
Use abcNormal for everything — specify size and weight, never change the font
Keep the interface invisible — no heavy borders, no shadows, no bright colors
Use the cool slate grays (#767d88, #7d848e) for secondary text — not warm grays
Uppercase labels need letter-spacing (0.35px) — never tight uppercase
Dark sections should be truly dark (#000000 or #1a1a1a) — no medium grays as surfaces
...
Add structure: Use `#212121` surfaces and `#353535` borders for containment -- no shadows
Apply typography: Inter (or Space Grotesk) with tight letter-spacing on headings, 1.50 line-height on body
Color punctuation: Add `#f36458` for CTAs and `#0052ef` for all hover/interactive states
...
Polish: Ensure all interactive elements hover to `#0052ef`, all buttons are pills or subtle 5px radius, borders are hairline (1px)
...
All text is uppercase with positive letter-spacing — no exceptions
Only two colors: black and spectral white (#f0f0fa)
...
Every section is full-viewport (100vh) — cinematic pacing
...
Iteration Guide
Start with #121212 — everything lives in near-black darkness
Spotify Green for functional highlights only (play, active, CTA)
Pill everything — 500px for large, 9999px for small, 50% for circular
Uppercase + wide tracking on buttons — the systematic label voice
Heavy shadows (0.3–0.5 opacity) for elevation — light shadows are invisible on dark
Album art provides all the color — the UI stays achromatic
...
Iteration Guide
Always enable `font-feature-settings: "ss01"` on sohne-var text -- this is the brand's typographic DNA
Weight 300 is the default; use 400 only for buttons/links/navigation
Shadow formula: `rgba(50,50,93,0.25) 0px Y1 B1 -S1, rgba(0,0,0,0.1) 0px Y2 B2 -S2` where Y1/B1 are larger (far shadow) and Y2/B2 are smaller (near shadow)
Heading color is `#061b31` (deep navy), body is `#64748d` (slate), labels are `#273951` (dark slate)
Border-radius stays in the 4px-8px range -- never use pill shapes or large rounding
Use `"tnum"` for any numbers in tables, charts, or financial displays
Dark sections use `#1c1e54` -- not black, not gray, but a deep branded indigo
SourceCodePro for code at 12px/500 with 2.00 line-height (very generous for readability)
...
Iteration Guide
Start with #171717 background — everything is dark-mode-native
Green is the brand identity marker — use it for links, logo, and accent borders only
Depth comes from borders (#242424 → #2e2e2e → #363636), not shadows
Weight 400 is the default for everything — 500 only for interactive elements
Hero line-height of 1.00 is the signature typographic move
Pill (9999px) for primary actions, 6px for secondary, 8-16px for cards
HSL with alpha channels creates the sophisticated translucent layering
...
Iteration Guide
When refining existing screens generated with this design system:
Verify font weight is 460 (not 400 or 500) for body and 540 for display — the non-standard weights are essential
Check that display line-height is 0.96 — if headlines look too spaced, they're wrong
Ensure buttons use Warm Cream (#e9e5dd) not pure white or gray — the warmth is subtle but critical
Confirm the only accent color is Lavender Glow (#cbb7fb) — no other hues should appear
The overall tone should feel like a luxury product presentation — minimal, confident, with one dramatic color gesture in the hero
...
Opacity-based transparency: The frosted glass nav and overlay modals use background-color opacity rather than shadow to indicate layering
Photography-as-depth: The full-bleed images create their own visual depth through perspective, lighting, and composition — making UI shadows redundant
Decorative Depth
...
Reference specific color names and hex codes from this document — there are only 6-7 colors in the entire system
Use natural language descriptions, not CSS values — "barely rounded corners" not "border-radius: 4px"
Describe the desired "feel" alongside specific measurements — "gallery-like silence between sections" communicates the whitespace philosophy better than "margin-bottom: 100vh"
Always verify that photography is doing the emotional heavy-lifting — if the UI itself feels "designed," it's too much
...
Dark sections use #010120 (midnight blue), never generic black
Shadows are always dark-blue-tinted: rgba(1, 1, 32, 0.1)
Mono labels are always uppercase with positive letter-spacing
Keep radius sharp (4px or 8px) — no pills, no generous rounding
Pastel gradients are for decoration, not UI chrome
...
Iteration Guide
Focus on ONE component at a time
Reference the strict black/white palette -- "use Uber Black (#000000)" not "make it dark"
Always specify 999px radius for buttons and pills -- this is non-negotiable for the Uber identity
Describe the font family explicitly -- "UberMove Bold for the heading, UberMoveText Medium for the label"
For shadows, use "whisper shadow (rgba(0,0,0,0.12) 0px 4px 16px)" -- never heavy drop shadows
Keep layouts compact and information-dense -- Uber is efficient, not airy
Illustrations should be warm and human -- describe "stylized people in warm tones" not abstract shapes
Pair black CTAs with white secondaries for balanced dual-action layouts
...
Ensure all buttons use the restrained dark palette (#353534) — no bright or colorful CTAs
Check that Matter Regular (400) is the default weight — Medium (500) only for emphasis
Confirm uppercase labels have wide letter-spacing (1.4px–2.4px) — tight uppercase feels wrong here
The overall tone should feel warm and calm, like a well-designed magazine — not aggressive or tech-flashy
...
Iteration Guide
Always start with `#1f2228` background -- never use pure black or gray backgrounds
GeistMono for display and buttons, universalSans for everything else -- never mix these roles
All buttons must be GeistMono uppercase with 1.4px letter-spacing -- this is non-negotiable
No shadows, ever -- depth comes from border opacity and background opacity only
Borders are always white with low opacity (0.1 default, 0.2 for emphasis)
Hover behavior dims to 0.5 opacity rather than brightening -- the reverse of most systems
Sharp corners (0px) by default -- only use 4px for specific secondary containers
Body text at 16px universalSans with 1.5 line-height for comfortable reading
Generous section padding (48px-96px) -- let content breathe in the darkness
The monochromatic white-on-dark palette is absolute -- resist adding color unless critical for function
